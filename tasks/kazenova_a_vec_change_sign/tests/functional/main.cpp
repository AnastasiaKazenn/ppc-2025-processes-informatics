...
class KazenovaAVecChangeSignFuncTests
...
const std::array<TestType, 3> kTestParam = {
    std::make_tuple(3, "alternating_signs"), std::make_tuple(5, "mixed_signs"),
    std::make_tuple(7, "same_signs")};

const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<KazenovaAVecChangeSignMPI, InType>(
                       kTestParam, PPC_SETTINGS_kazenova_a_vec_change_sign),
                   ppc::util::AddFuncTask<KazenovaAVecChangeSignSEQ, InType>(
                       kTestParam, PPC_SETTINGS_kazenova_a_vec_change_sign));

const auto kGtestValues = ppc::util::ExpandToValues(kTestTasksList);

const auto kPerfTestName = KazenovaAVecChangeSignFuncTests::PrintFuncTestName<
    KazenovaAVecChangeSignFuncTests>;

INSTANTIATE_TEST_SUITE_P(VecChangeSignTests, KazenovaAVecChangeSignFuncTests,
