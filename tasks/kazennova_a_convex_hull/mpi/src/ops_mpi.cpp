    ...
    if (world_rank == 0) {
        ...
        for (int i = 0; i < world_size; ++i) {
            ...
            if (i == 0) {
                ...
                MPI_Send(all_points.data() + start, count * sizeof(Point), MPI_BYTE, 
                        i, 1, MPI_COMM_WORLD);
            }
            start += count;
        }
        ...
        MPI_Recv(local_points_.data(), count * sizeof(Point), MPI_BYTE, 
...
std::vector<Point> KazennovaAConvexHullMPI::GatherLocalHulls() {
    ...
    if (world_rank == 0) {
        ...
        for (int i = 1; i < world_size; ++i) {
            MPI_Recv(all_hull_points.data() + displs[i], sizes[i] * sizeof(Point), MPI_BYTE,
