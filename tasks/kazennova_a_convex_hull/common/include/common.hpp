...
inline bool polar_angle(const Point& pivot, const Point& a, const Point& b) {
...
template <typename T, typename Compare>
void sort_quick(std::vector<T>& a, int low, int high, Compare comp) {
    if (low < high) {
        int pi = partition(a, low, high, comp);
        sort_quick(a, low, pi - 1, comp);
        sort_quick(a, pi + 1, high, comp);
    }
}

template <typename T>
size_t find_min_index(const std::vector<T>& a) {
    size_t min_idx = 0;
...
template <typename T>
void copy_elements(const std::vector<T>& src, std::vector<T>& dst, size_t dst_start) {
